...
public:
    ...
    static std::optional<comparable_bytes> from_managed_bytes_view(const abstract_type& type, const managed_bytes_view_opt& mbv) {
        if (!mbv) {
            return std::nullopt;
        }
    ...
    static std::optional<comparable_bytes> from_managed_bytes(const abstract_type& type, const managed_bytes_opt& mb) {
        return from_managed_bytes_view(type, managed_bytes_view_opt(mb));
    }
    static std::optional<comparable_bytes> from_bytes_view(const abstract_type& type, std::optional<bytes_view> bv) {
        return from_managed_bytes_view(type, managed_bytes_view_opt(bv));
    }
    static std::optional<comparable_bytes> from_data_value(const data_value& value);
